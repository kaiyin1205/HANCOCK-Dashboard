...
def clip_biomarkers_on_train(X_train_df, X_test_df, cols):
    ...
    for col in cols:
        if col not in X_train_df.columns:
            print(f"  [CLIP] WARNING: '{col}' not found, skipping.")
            continue
...
def return_optimal_extra_trees(target='recurrence', random_state=np.random.RandomState(42), data_split='In distribution'):
    """Returns the Extra Trees classifier with tuned hyperparameters"""
...
if __name__ == "__main__":
    ...
    for i in range(len(data_split_paths)):
        ...
        print(f"  Train: {len(df_train)} patients | class balance: {df_train['target'].value_counts().to_dict()}")
        print(f"  Test : {len(df_test)} patients  | class balance: {df_test['target'].value_counts().to_dict()}")
        ...
        for iteration in range(5):
            ...
            # ── 訓練 Extra Trees 模型 ──────────────────────
            model = return_optimal_extra_trees(target=target, data_split=data_split_labels[i])
            ...
            if iteration == 4:
                ...
                sns.barplot(x=importances[top_indices],
                            y=[selected_feature_names[idx] for idx in top_indices],
                            palette="magma",
                            hue=[selected_feature_names[idx] for idx in top_indices],
                            legend=False)
                plt.title(f"Top {top_n} - {data_split_labels[i]} Feature Importance (ExtraTrees)")

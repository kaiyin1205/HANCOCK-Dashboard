...
if not st.session_state["disclaimer_accepted"]:
    st.title("🏥 HANCOCK Risk System")
    st.markdown("---")
    st.warning(
        "IMPORTANT DISCLAIMER\n\n"
        "This system is designed to assist — not replace — professional medical judgment. "
        "Risk assessments provided are for reference only and should not be used as the sole basis for clinical decisions.\n\n"
        "By using this system, you acknowledge that:\n"
        "- Results are generated by a machine learning model and may not be accurate\n"
        "- All clinical decisions must be made by a qualified healthcare professional\n"
        "- This tool does not constitute medical advice or diagnosis"
    )
    ...
    with st.form("doctor_form"):
        ...
        with col2:
            ...
            neutrophils = st.number_input(
                "Neutrophils (x10^3/uL)",
                min_value=0.0,
                value=float(st.session_state.get("doc_neutrophils", 4.0)),
                step=0.1
            )
            lymphocytes = st.number_input(
                "Lymphocytes (x10^3/uL)",
                min_value=0.1,
                value=float(st.session_state.get("doc_lymphocytes", 2.0)),
                step=0.1
            )
            platelets = st.number_input(
                "Platelets (x10^3/uL)",
                min_value=0.0,
                value=float(st.session_state.get("doc_platelets", 200.0)),
                step=1.0
            )
            monocytes = st.number_input(
                "Monocytes (x10^3/uL)",
                min_value=0.1,
                value=float(st.session_state.get("doc_monocytes", 0.5)),
                step=0.1
            )
                ...
                with col_r2:
                    ...
                    with st.spinner("Generating clinical recommendation..."):
                        try:
                            ...
                            st.info(ai_rec)
                            st.warning(
                                "Literature references above are AI-generated and may not be accurate. "
                                "Please verify all citations independently before clinical use. "
                                "Suggested databases: PubMed (pubmed.ncbi.nlm.nih.gov), "
                                "Cochrane Library, or Google Scholar."
                            )
                        except Exception as e:
